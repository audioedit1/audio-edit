...
// =====================
// SAMPLE LIBRARY PREVIEW AUDIO (ISOLATED — EXISTING MARKUP ONLY)
// =====================
// Contract notes:
// - No new import UI, no dynamic card creation.
// - Attaches only to existing `.sample-card` elements already present in the DOM.
// - Editor WaveSurfer + export DSP logic above remains untouched.

const samplePreviewControllers = new Set();

// Shared Preview AudioContext for the entire Sample Library preview system.
// Must remain isolated from the editor audio engine.
const samplePreviewAudioContext = new AudioContext();

function stopAllSamplePreviews(exceptController) {
  samplePreviewControllers.forEach(controller => {
...
function wireSampleCardPreview(cardEl) {
  const src = findFirstAudioSource(cardEl);
  if (!src) return null;

  const waveEl =
    cardEl.querySelector("[data-preview-waveform]") ||
    cardEl.querySelector(".sample-card__wave");
  ...
  const volInput =
    cardEl.querySelector("[data-preview-volume]") ||
    cardEl.querySelector('.sample-card__meta input[type="range"]') ||
    null;
  ...
  const previewWaveSurfer = WaveSurfer.create({
    ...
    autoScroll: false,
    interact: true,
    audioContext: samplePreviewAudioContext
  });

  previewWaveSurfer.load(src);

  function syncPlayButton() {
    if (!playBtn) return;
    playBtn.textContent = previewWaveSurfer.isPlaying?.() ? "Pause" : "Play";
  }

  const controller = {
    stop() {
      previewWaveSurfer.stop();
      syncPlayButton();
    },
    destroy() {
      try {
      ...
      samplePreviewControllers.delete(controller);
    }
  };

  samplePreviewControllers.add(controller);

  previewWaveSurfer.on("ready", () => {
    if (!durationEl) return;
    const dur = Number(previewWaveSurfer.getDuration?.() || 0);
    const total = Number.isFinite(dur) && dur >= 0 ? Math.floor(dur) : 0;
    const mins = Math.floor(total / 60);
    const secs = total % 60;
    durationEl.textContent = `${mins}:${String(secs).padStart(2, "0")}`;
  });

  previewWaveSurfer.on("play", syncPlayButton);
  previewWaveSurfer.on("pause", syncPlayButton);
  previewWaveSurfer.on("finish", () => {
    if (loopInput?.checked) {
      previewWaveSurfer.play(0);
      return;
    }
    syncPlayButton();
  });
  ...
  if (playBtn) {
    playBtn.addEventListener("click", async () => {
      stopAllSamplePreviews(controller);

      try {
        await samplePreviewAudioContext.resume();
      } catch {
        // best-effort
      }
  ...
  if (volInput) {
    volInput.addEventListener("input", e => {
      const value = Number(e.target.value);
      previewWaveSurfer.setVolume(Number.isFinite(value) ? value : 1);
    });
  }
...
window.addEventListener("beforeunload", () => {
  ...
  try {
    samplePreviewAudioContext.close();
  } catch {
    // best-effort
  }
});
